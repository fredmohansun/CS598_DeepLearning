...
max_epoch = 20 if args.max_epoch is None else args.max_epoc
...
switches = [args.nnDropout,args.twice,args.CrossEntropy]
savefile = '_'.join(['2a', 'ADAM' if args.ADAM else 'SGD', str(lr), str(max_epoch), str(batch_size), str(no_of_hidden_units), str(vocab_size), ''.join(list(map(str,map(int,switches)))), str(train_sl),str(test_sl)]) + '.out'
print(savefile)
...
torch.save(model, 'model/'+savefile.replace('.out','.model'))
